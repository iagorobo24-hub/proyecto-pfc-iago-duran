...
test.describe('Theme Audit — Light & Dark Mode', () => {
  ...
  test('fichas — sidebar search visible in both themes', async ({ page }) => {
    await page.goto(`${BASE}/app/fichas`, { waitUntil: 'networkidle', timeout: 20000 })
    await page.waitForTimeout(2000)
  ...
  test('sonex — all mode tabs work', async ({ page }) => {
    await page.goto(`${BASE}/app/sonex`, { waitUntil: 'networkidle', timeout: 20000 })
    await page.waitForTimeout(2000)

    await checkPageContent(page)
    await expect(page.getByPlaceholder('Escribe tu consulta técnica...')).toBeVisible({ timeout: 10000 })

    const modeLabels = ['🔍 Búsqueda', '⚖️ Comparativa', '🤝 Asistencia', '📚 Formación']
    for (const label of modeLabels) {
      const tab = page.getByRole('button', { name: label, exact: false })
    ...
    await page.waitForTimeout(500)
    await checkPageContent(page)
    await expect(page.getByPlaceholder('Escribe tu consulta técnica...')).toBeVisible()
  })

  test('almacen — simulation flow loads', async ({ page }) => {
    await page.goto(`${BASE}/app/almacen`, { waitUntil: 'networkidle', timeout: 20000 })
    await page.waitForTimeout(2000)
  ...
  test('presupuestos — wizard loads and buttons visible', async ({ page }) => {
    await page.goto(`${BASE}/app/presupuestos`, { waitUntil: 'networkidle', timeout: 20000 })
    await page.waitForTimeout(2000)
...
test.describe('Theme Audit — Button Contrast', () => {
  ...
  test('kpi — primary and secondary buttons visible in both themes', async ({ page }) => {
    await page.goto(`${BASE}/app/kpi`, { waitUntil: 'networkidle', timeout: 20000 })
    await page.waitForTimeout(2000)
