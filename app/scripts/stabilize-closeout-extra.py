...
# Browser tests must be credentialless and deterministic. CI uses Playwright's
# standard Chromium; only isolate the dev webServer from any real Supabase env.
playwright_path = ROOT / 'app/playwright.config.js'
playwright = playwright_path.read_text()
old = "    timeout: 120_000,\n  },"
new = "    timeout: 120_000,\n    env: {\n      ...process.env,\n      VITE_SUPABASE_URL: '',\n      VITE_SUPABASE_ANON_KEY: '',\n    },\n  },"
if old not in playwright:
    raise SystemExit('Playwright webServer marker not found')
playwright_path.write_text(playwright.replace(old, new, 1))
